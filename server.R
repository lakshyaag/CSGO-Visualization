...
grenadeTrajectoryPlot <-
    function(map_focus, player, data) {
        map_x = mapCoordinates %>% filter(map_name == map_focus) %>% pull(x)
        map_y = mapCoordinates %>% filter(map_name == map_focus) %>% pull(y)
        scale = mapCoordinates %>% filter(map_name == map_focus) %>% pull(scale)
...
shinyServer(function(input, output) {
    ...
    maps <-
        reactive({
            data() %>% select(mapName) %>% unique() %>% pull()
        })
    teams <-
        reactive({
            data() %>% select(throwerTeam) %>% unique() %>% pull()
        })
    players <-
        reactive({
            data() %>% select(throwerName) %>% unique() %>% pull()
        })
